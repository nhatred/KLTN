...
import Question from  '../models/Question.js'; 
  ...
  async function submitAnswer(socket, data) {
    try {
      ...
      const questionExists = participant.remainingQuestions.some(q => q.equals(questionId));
      if (!questionExists) {
      ...
      // Tính điểm
      const { isCorrect, score } = this._evaluateAnswer(question, answer);
      ...
      // Gửi sự kiện cập nhật về client
        socket.broadcast.to(`participant_${participant._id}`).emit('questionsUpdate', {
  ...
  async function syncSubmissions(participantId) {
    try {
      ...
      const currentQuestion = participant.completedQuestions[participant.currentQuestionIndex].questionId;
