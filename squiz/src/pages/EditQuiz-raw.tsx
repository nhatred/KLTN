...
export default function EditQuiz() {
  ...
  const saveQuiz = async (formData: any) => {
    ...
    try {
      ...
      // Update user with quiz
      if (userId) {
        await updateUserWithQuiz(userId, quizId, token);
      }
  ...
  const updateUserWithQuiz = async (
    userId: string,
    quizId: string,
    token: string
  ) => {
    try {
      ...
      if (!response.ok) throw new Error("Failed to update user with quiz");

      return await response.json();
    } catch (error) {
      console.error("Error updating user with quiz:", error);
      throw error;
    }
  };
          ...
          <div className="col-span-5">
            ...
            <div className="w-full px-8 py-5 bg-white rounded-lg col-span-5 box-shadow">
              ...
              <div className="mt-5">
                <input
                  {...register("imageUrl", {
                    required: !quiz?.imageUrl && "Hình ảnh phải có",
                  })}
                  accept="image/*"
                  className="mb-2"
                  type="file"
